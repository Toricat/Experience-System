...
class AuthService:
        ...
        async def active_code_by_email_service(self, session, data: VerifyEmailSend):
            ...
            verify_active = await crud_verify.get(session, columns=["id "," exp_active"],user_id=user.id)
            ...
            if verify_active is not None and verify_active.exp_active < datetime.utcnow():
                await crud_verify.update(session, id=verify_active.id, obj_in=new_verify_active.dict(exclude=None))
            else:
                await crud_verify.create(session , obj_in=new_verify_active)

            html_content = await render_email_template( "verify_code.html",verification_code= new_code_active )
        ...
        async def recovery_code_by_email_service(self, session, data: VerifyEmailSend):
            ...
            verify_recovery = await crud_verify.get(session, columns=["id "," exp_recovery"],user_id=user.id)

            if verify_recovery is not None and verify_recovery.exp_recovery > datetime.utcnow():
                return SuccessResponse("Already send code. Please check your email to verify your account.")
            
            new_code_recovery, new_exp_recovery = create_verify_code()
            new_verify_recovery = ActivateCodeInDB(code_recovery=new_code_recovery,exp_recovery=new_exp_recovery, user_id=user.id)

            if verify_recovery is not None and verify_recovery.exp_recovery < datetime.utcnow():
                await crud_verify.update(session, id=verify_recovery.id, obj_in=new_verify_recovery.dict(exclude=None))
            else:
                await crud_verify.create(session , obj_in=new_verify_recovery)

            html_content = await render_email_template( "verify_code.html",verification_code= new_code_recovery )
            response = await send_email(
                email_to=data.email,
        ...
        async def confirm_active_code(self, session, data: VerifyCodeComfirm):
            verify_active = await crud_verify.get(session,columns=["exp_active"], code_active=data.verify_code)
            print(verify_active)
        ...
        async def confirm_active_code_account_service(self, session, data: VerifyCodeComfirm):
            ...
            await crud_user.update(session, id=verify_active.user_id, obj_in=obj_in)
            return SuccessResponse("User Activated")

        @handle_error
        async def confirm_recovery_code(self, session, data: VerifyCodeComfirm):
            verify_recovery = await crud_verify.get(session,columns=["exp_recovery"], code_recovery=data.verify_code)
        ...
        async def confirm_recovery_code_service(self, session, data: VerifyCodeComfirm):
            verify_recovery = await self.confirm_active_code(session,data)
